    ...
    <el-container>
      <el-aside width="400px" class="asidebar">
        <el-menu default-active="1" class="el-menu-demo" mode="horizontal" @select="handleSelect">
          ...
          <el-menu-item index="3" style="padding:0px 12px">已销售</el-menu-item>
          <el-menu-item index="4" style="padding:0px 13px">已撤销</el-menu-item>
          <el-menu-item index="5" style="padding:0px 13px">待出库</el-menu-item>
      ...
      <el-container>
        <el-header class="headerbar">
          <el-row>
            <el-button type="primary" icon="el-icon-plus" @click="addPactItem">新增销售单</el-button>
          </el-row>
        </el-header>
        <el-main class="mainbar">
          <div class="pickPactItembar">
            <el-form ref="pickPactItem" :model="pickPactItem" label-width="120px">
              <el-row>
                <el-col :span="12">
                  <el-form-item label="出库仓库">
                    <el-select
                      v-model="pickPactItem.warehouseId"
                      clearable
                      placeholder="请选择出库仓库"
                      :disabled="!edit"
              ...
              <el-row>
                ...
                <el-col :span="12">
                  <el-form-item label="付款金额">
                    <el-input
                      placeholder="请输入内容"
                      v-model.number="pickPactItem.payPrice"
                      clearable
                      v-if="edit"
                      style="width:80%"
                    ></el-input>
                    <span v-else>{{pickPactItem.payPrice}}</span>
                  </el-form-item>
                </el-col>
                </el-row>
              <el-row>
                <el-col :span="12">
                  <el-form-item label="销售日期">
                    <el-date-picker
                      v-model="pickPactItem.pactDate"
                      type="date"
                      placeholder="选择日期"
                      v-if="edit"
                      style="width:80%"
                    ></el-date-picker>
                    <span v-else>{{pickPactItem.pactDate}}</span>
                  </el-form-item>
                </el-col>
                <el-col :span="24">
                  <el-form-item label="备注">
                    <el-input
                      placeholder="请输入内容"
                      v-model.number="pickPactItem.pactText"
                      clearable
                      v-if="edit"
                      type="textarea"
                    ></el-input>
                    <span v-else>{{pickPactItem.pactText}}</span>
                  </el-form-item>
                </el-col>
              </el-row>
            </el-form>
          ...
          <div class="pactitemlist">
            <vxe-toolbar custom>
              <template v-slot:buttons>
                <vxe-button
                  icon="vxe-icon--plus"
                  v-if="savemode == 1 || savemode == 0"
                  vxe-icon--plus
                  status="primary"
                  @click="insertpickPactItem"
                >新增销售项目</vxe-button>
              </template>
            </vxe-toolbar>
            ...
            >
              ...
              <vxe-table-column field="pickNumber" title="数量"></vxe-table-column>
              <vxe-table-column field="pickPrice" title="单价"></vxe-table-column>
              <vxe-table-column field="pickTotalPrice" title="总价"></vxe-table-column>
          ...
          <div class="footbar" v-if="showbuttom">
            ...
            <div class="footbtn2" v-else-if="savemode == 4">
              <el-button type="primary" @click="addRefundPact">生成退货单</el-button>
            </div>
          </div>
        </el-main>
      </el-container>
          ...
          <el-row>
            ...
            <el-col :span="12">
              <el-form-item label="数量">
                <el-input-number
                  v-model="packeachItem.pickNumber"
...
import {
  ...
  saveOrModifyPickPact,
  getPickPactById,
  removePickPact
} from "@/api/wms/pickpact/pickpact";
import {
  getPickBillPageByAttributes,
  saveOrModifyPickBill,
  saveOrModifyBatchPickBill,
  removeBatchPickBill,
  removePickBill
...
import { getStoragePageByAttributes } from "@//api/wms/storage/storage";
export default {
  name: "pick-order",
  //   computed: {
  //   ...mapGetters(["name"])
  ...
  methods: {
    ...
    _getPickPactPageByAttributes(params) {
      this.pickPactlist = [];
      getPickPactPageByAttributes(params).then(res => {
        if (res.status === 200) {
    ...
    _getPickPactById(pactId) {
      getPickPactById(pactId).then(res => {
        if (res.status === 200) {
          this.pickPactItem = res.data;
          this.choicebuttonmode(this.pickPactItem);
        }
      });
    },
    _getPickBillPageByAttributes(params) {
      getPickBillPageByAttributes(params).then(res => {
        if (res.status === 200) {
    ...
    _saveOrModifyPickPact(data, callback) {
      saveOrModifyPickPact(data).then(res => {
        if (res.status === 200) {
          this.$message({
            message: "数据保存成功！",
            type: "success"
    ...
    _saveOrModifyPickBill(data, callback) {
      saveOrModifyPickBill(data).then(res => {
        if (res.status === 200) {
          this.$message({
            message: "数据保存成功！",
            type: "success"
    ...
    _saveOrModifyBatchPickBill(data, callback) {
      saveOrModifyBatchPickBill(data).then(res => {
        if (res.status === 200) {
          this.$message({
            message: "数据保存成功！",
            type: "success"
    ...
    _getpactList() {
      ...
      this._getPickPactPageByAttributes(this.searchItem);
    },

    _getPactItemPage() {
      let param = {
        pactId: this.pickPactItem.pactId,
        current: this.pactItemqueryList.current,
        size: this.pactItemqueryList.size
      };
      this._getPickBillPageByAttributes(param);
    },
    // 商品分类懒加载
    ...
    _removePickPact(pickpactId) {
      removePickPact(pickpactId).then(res => {
        if (res.status === 200) {
          this.$message({
            message: "数据删除成功！",
            type: "success"
    ...
    _removeBatchPickBill(data) {
      removeBatchPickBill(data).then(res => {
        if (res.status === 200) {
          this.$message({
            message: "数据删除成功！",
            type: "success"
    ...
    _removePickBill(pickBillId) {
      removePickBill(pickBillId).then(res => {
        if (res.status === 200) {
          this.$message({
            message: "数据删除成功！",
            type: "success"
    ...
    // 插入货物项
    insertpickPactItem() {
      if (this.pickPactItem.warehouseId != null) {
    ...
    itemdetailselect() {
      this._getItemDetailById(this.packeachItem.detailId, data => {
        ...
        this.packeachItem.pickPrice = data.detailPrice;
        this.packeachItem.pickTax = data.detailTax;
        this.packeachItem.pickType = data.detailType
        let params = {
          warehouseId: this.pickPactItem.warehouseId,
    ...
    comfirmPactItem() {
      ...
      if (this.pactItemList.length > 0) {
        // 判断是不是修改原有内容
        if (this.packeachItem.pickBillId) {
          for (var i = 0; i < this.pactItemList.length; i++) {
            if (
              this.pactItemList[i].pickBillId == this.packeachItem.pickBillId
            ) {
          ...
          if (info.status == "edit" && !this.packeachItem._XID) {
            ...
            this.pactItemList[info.num].pickNumber =
              this.packeachItem.pickNumber +
              this.pactItemList[info.num].pickNumber;
              if(this.pactItemList[info.num].pickNumber > this.maxnum){
                this.$message("累计出库数量大于库存数量！默认修改为库存数量。");
                this.pactItemList[info.num].pickNumber = this.maxnum
              }
    ...
    getsupplierName() {
      for (var i = 0; i < this.supplierList.length; i++) {
        if (this.supplierList[i].supplierId == this.pickPactItem.supplierId) {
          this.pickPactItem.suppilerName = this.supplierList[i].supplierName;
        }
      }
    },
    getwarehouseName() {
      for (var i = 0; i < this.warehouseList.length; i++) {
        if (
          this.warehouseList[i].warehouseId == this.pickPactItem.warehouseId
        ) {
          this.pickPactItem.warehouseName = this.warehouseList[i].warehouseName;
        }
      }
    },
    // 编辑货物条目操作
    ...
    deleteRowEvent(row) {
      if (row.pickBillId) {
        ...
        this._removePickBill(row.pickBillId)
      } else {
        for (var i = 0; i < this.pactItemList.length; i++) {
          if (this.pactItemList[i].detailId == row.detailId) {
            this.pactItemList.splice(i, 1);
    ...
    choicebuttonmode(data) {
      if (data.pactClassify == "WMS_PICKPACT_DRIFT") {
        this.showbuttom = true;
        this.edit = true;
        this.savemode = 1;
        // 采购完成
      } else if (data.pactClassify == "WMS_PICKPACT_FINISH") {
        this.showbuttom = true;
        this.edit = false;
        this.savemode = 4;
        // 待入库操作
      } else if (data.pactClassify == "WMS_PICKPACT_WAITSAVE") {
        this.showbuttom = true;
        this.edit = false;
        this.savemode = 2;
        // 部分入库操作
      } else if (data.pactClassify == "WMS_PICKPACT_SOMESAVE") {
        this.showbuttom = true;
        this.edit = false;
        this.savemode = 3;
      }
    ...
    async saveNewPunPact() {
      if (this.pickPactItem.warehouseId == null) {
        ...
        this.pickpactId = null;
        this.pickPactItem.pactClassify = "WMS_PICKPACT_DRIFT";
        // 保存或修改合同信息
        await this._saveOrModifyPickPact(this.pickPactItem, data => {
          this.pickpactId = data.pactId;
        });
        setTimeout(() => {
          for (var i = 0; i < this.pactItemList.length; i++) {
            this.pactItemList[i].pactId = this.pickpactId;
            this.pactItemList[i].warehouseId = this.pickPactItem.warehouseId;
            this.pactItemList[
              i
            ].warehouseName = this.pickPactItem.warehouseName;
          }
          // 保存或修改条目信息
          this._saveOrModifyBatchPickBill(this.pactItemList, data => {});
          setTimeout(() => {
            // 保存成功后获取合同信息
            this._getPickPactById(this.pickpactId);
            let param = {
              pactId: this.pickpactId,
              current: this.pactItemqueryList.current,
              size: this.pactItemqueryList.size
            };
            // 保成功后获取条目信息
    ...
    async setPickPactClassify() {
      if (this.pickPactItem.warehouseId == null) {
        ...
        this.pickpactId = null;
        this.pickPactItem.pactClassify = "WMS_PICKPACT_WAITSAVE";
        // 保存或修改合同信息
        await this._saveOrModifyPickPact(this.pickPactItem, data => {
          this.pickpactId = data.pactId;
        });
    ...
    removePunPact() {
      this.$confirm("此操作将永久删除该订单及订单下条目, 是否继续?", "提示", {
        ...
        .then(() => {
          this._removePickPact(this.pickPactItem.pactId);
          this.pickBillIdList = [];
          for (var i = 0; i < this.pactItemList.length; i++) {
            this.pickBillIdList.push(this.pactItemList[i].pickBillId);
          }
          this._removeBatchPickBill(this.pickBillIdList);
          setTimeout(() => {
            this.addPactItem();
            this._getpactList();
          }, 500);
    ...
    handleSelect(key, keyPath) {
      ...
      if (key === "1") {
        ...
        this.pickPactlist = [];
        this.searchItem.pactClassify = "WMS_PICKPACT_DRIFT";
        this._getpactList();
      } else if (key === "3") {
        this.pickPactlist = [];
        this.searchItem.pactClassify = "WMS_PICKPACT_FINISH";
        this._getpactList();
      } else if (key === "4") {
        this.pickPactlist = [];
        this.searchItem.pactClassify = "WMS_PICKPACT_ABANDON";
        this._getpactList();
      } else if (key === "5") {
        this.pickPactlist = [];
        this.searchItem.pactClassify = "WMS_PICKPACT_WAITSAVE";
        this._getpactList();
      } else if (key === "6") {
        this.pickPactlist = [];
        this.searchItem.pactClassify = "WMS_PICKPACT_SOMESAVE";
        this._getpactList();
      }
    },
    // 格式化时间
    ...
    getTotalPrice(currentValue) {
      this.packeachItem.pickTotalPrice =
        currentValue * this.packeachItem.pickPrice;
    ...
    addAllInPact() {
      ...
      //   }
      //   this._purchaseInStorage(dataList,this.pickPactItem.warehouseId,(data)=>{
      //     console.info(data)
      //   })
      ...
      this.$router.push({
        ...
        params: { id: this.pickPactItem.pactId }
      });
    },
